...
group_by_top_100_parts_v2_pushdown_limit <- function(datasource) {
  druid.query.groupBy(
    url = url,
    dataSource   = datasource,
    intervals    = i,
    ...
    orderBy=list("l_partkey"),
    limit=100,
    context=list(useCache=F, populateCache=F, groupByStrategy = 'v2', pushDownLimit=T, typeHints=list(l_partkey="long")),
    verbose = TRUE,
    benchmark = TRUE
  )
}

top_100_parts_typed <- function(datasource) {
  druid.query.topN(
    url = url,
    dataSource   = datasource,
    intervals    = i,
    ...
    filter = NULL,
    granularity = granularity("all"),
    context=list(useCache=F, populateCache=F, typeHints=list(l_partkey="long")),
    verbose = TRUE,
    benchmark = TRUE
  )
}
...
group_by_top_100_shipdate_v2 <- function(datasource) {
  druid.query.groupBy(
    url = url,
    dataSource   = datasource,
    intervals    = i,
    ...
    granularity = granularity("all"),
    dimensions = list("l_shipdate"),
    orderBy=list("l_shipdate"),
    limit=100,
    context=list(useCache=F, populateCache=F, groupByStrategy = 'v2'),
    verbose = TRUE,
    benchmark = TRUE
...
res16 <- microbenchmark(group_by_top_100_shipdate_v2(datasource), times=n)
...
results <- as.data.frame(rbind(res16))
